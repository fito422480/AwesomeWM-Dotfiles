...
local function make_launcher(opts)
    local launcher = wibox.widget {
        markup = opts.markup,
        align = 'center',
        font = opts.font and opts.font or beautiful.nerd_font .. ' 24',
...
local spotify = make_launcher {
    markup = '',
    onclick = function ()
        donotif('spotify')
        awful.spawn(spotify) -- comes from `user_likes.lua`
    end
}

local code = make_launcher {
    markup = '',
    onclick = function ()
        donotif('code')
        awful.spawn(visual_editor) -- comes from `user_likes.lua`
    end
}
